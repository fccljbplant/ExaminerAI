...
echo "  console.log:    $(git grep 'console.log\|console.error\|console.warn' -- src 2>/dev/null | grep -v 'logger.ts' | grep -v '__tests__' | wc -l)"
echo "  any types:      $(git grep ': any\b\|as any\|<any>' -- src 2>/dev/null | grep -v node_modules | grep -v __tests__ | grep -v '// .*any' | grep -v '\* .*any' | wc -l)"
