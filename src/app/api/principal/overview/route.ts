...
export async function GET() {
  ...
  const [institution, studentsCount, teachersCount, counselorsCount, mentorsCount, coursesCount, alerts, mentorSessionsCount, wellbeingStates, crisisFlags, healthSummaries, auditLogs, growthReports, enrollmentsCount, courses, teachers] = await Promise.all([
  ...
  const coursePerformance = courses.map(c => ({
    ...
    code: c.name,
    name: c.name,
    teacher: "—", // We don't track per-course instructor yet
    studentCount: 0,
    avgScore: 0,
  }));
  ...
  const instructorEnrollments = await db.courseEnrollment.findMany({
    where: { userId: { in: instructorIds }, role: "instructor" },
    select: { userId: true, courseId: true },
  });
  const courseCountByInstructor = new Map<string, number>();
  for (const e of instructorEnrollments) {
    courseCountByInstructor.set(e.userId, (courseCountByInstructor.get(e.userId) || 0) + 1);
  }
  const teacherPerformance = teachers.map(t => ({
    id: t.id,
    name: t.name,
    email: t.email,
    courses: courseCountByInstructor.get(t.id) || 0,
    sessions: 0,
  ...
  return NextResponse.json({
    ...
    overview: { totalStudents: studentsCount, totalTeachers: teachersCount, totalCounselors: counselorsCount, totalMentors: mentorsCount, totalCourses: coursesCount, totalEnrollments: enrollmentsCount, totalAlerts: alerts.length, openAlerts: openAlerts.length, crisisFlags: crisisFlags.length, mentorSessions: mentorSessionsCount, avgMood, avgEngagement, totalFrustration, totalAvoidance, totalEnthusiasm },
