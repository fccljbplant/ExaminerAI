  ...
  return (
    <div className="rounded-lg border border-brand/30 bg-brand/5 p-3 space-y-3">
      ...
      <div className="flex items-center gap-2">
        ...
        <p className="text-xs font-semibold text-fg">What a strong answer looked like</p>
      </div>
      <p className="text-xs text-fg-muted leading-relaxed">{feedback.modelAnswer}</p>
      ...
      {feedback.missedPoints.length > 0 && (
        <>
          <div className="flex items-center gap-2 pt-1">
            ...
            <p className="text-xs font-semibold text-fg">What you could have added</p>
          </div>
      ...
      <div className="rounded-md bg-bg/70 border border-line p-2 mt-1">
    ...
    <div className="pt-2">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-fg-muted">({explanations.length})</span>
        </div>
        <button
          onClick={toggleAll}
      ...
      <div className="space-y-2">
        {explanations.map((q, i) => {
          ...
          return (
            <div key={i} className="rounded-md border border-line bg-bg/60 overflow-hidden">
              {/* Header — click to expand/collapse */}
              <button
                onClick={() => toggle(i)}
                className="w-full text-left px-3 py-2 flex items-start gap-2 hover:bg-bg-subtle/40 transition-colors"
              >
                {isOpen
                  ? <ChevronDown className="h-3 w-3 text-fg-muted flex-shrink-0 mt-0.5" />
                  : <ChevronRight className="h-3 w-3 text-fg-muted flex-shrink-0 mt-0.5" />}
                <div className="flex-1 min-w-0">
                  <p className="text-[10px] font-medium text-brand mb-0.5">Q{q.questionIndex + 1}</p>
                  <p className="text-xs text-fg line-clamp-2">{q.question}</p>
                </div>
              </button>

              {/* Body — only when expanded */}
              {isOpen && (
                <div className="px-3 pb-3 pt-1 space-y-2 border-t border-line/50">
                  {/* Student's answer */}
                  <div>
                    <p className="text-[10px] font-semibold text-fg-muted uppercase tracking-wider mb-0.5">Your answer</p>
                    <p className="text-xs text-fg-muted italic">&ldquo;{q.studentAnswer}&rdquo;</p>
                  </div>
                  ...
                  <div className="rounded-md bg-growth-sage-soft border border-growth-sage p-2">
                    ...
                    <p className="text-xs text-fg">{q.correctAnswer}</p>
                  </div>

                  {/* Why — explanation */}
                  <div>
                    <p className="text-[10px] font-semibold text-fg-muted uppercase tracking-wider mb-0.5">Why</p>
                    <p className="text-xs text-fg leading-relaxed">{q.explanation}</p>
                  </div>

                  {/* Encouragement */}
                  <div className="flex items-start gap-1.5 pt-1">
                    <Heart className="h-3 w-3 text-pink-500 flex-shrink-0 mt-0.5" />
                    <p className="text-xs text-fg italic">{q.encouragement}</p>
                  </div>
                </div>
              )}
            </div>
