...
/** Shape mirrored from GET /api/v2/org/home */
interface OrgHomeData {
  ...
  lowCompletionCourses?: number;
  pendingInvites?: number;
  recentUsers?: Array<{ name: string; role: string; program: string; activity: string; status: string }>;
}
  ...
  return (
    <>
      ...
      <div className="v3-grid v3-grid-4">
        <V3StatCard title="Active learners" value={String(d.activeLearners ?? 0)} label="Across all members" />
      ...
      <div className="v3-grid v3-grid-2">
        <V3Card>
          ...
          <div style={{ marginTop: 24 }}>
            ...
            <div style={{ margin: "7px 0 15px" }}><V3Progress value={d.activeRate ?? 0} /></div>
            <p>Completed · {d.completed ?? 0}</p>
            <div style={{ margin: "7px 0" }}><V3Progress value={d.completionRate ?? 0} /></div>
          </div>
        </V3Card>
      ...
      <V3SectionTitle title="Recent users" linkHref="/org/people" linkLabel="View all →" />
            ...
            {(d.recentUsers ?? []).length === 0 ? (
              ...
              d.recentUsers!.map((u, i) => (
                <tr key={i}>
                  ...
                  <td>{u.activity}</td>
                  <td>
                    <span className={`v3-badge v3-badge-${u.status === "Active" ? "success" : "warning"}`}>{u.status}</span>
                  </td>
                </tr>
              ))
            )}
