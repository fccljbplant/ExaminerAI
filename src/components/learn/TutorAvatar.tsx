...
function useTutorState() {
  ...
  useEffect(() => {
    ...
    const a = tutor.on("gesture", (g: Gesture) => {
      setGesture(g); clearTimeout(t1);
      if (ONE_SHOT[g]) t1 = setTimeout(() => setGesture("idle"), ONE_SHOT[g]);
    });
    const b = tutor.on("tts", () => setGesture("talk"));
    const c = tutor.on("tts:end", () => setGesture("idle"));
    const d = tutor.on("caption", (text: string) => {
      setCaption(text); clearTimeout(t2); t2 = setTimeout(() => setCaption(""), 6000);
    });
    return () => { a(); b(); c(); d(); };
  }, []);
