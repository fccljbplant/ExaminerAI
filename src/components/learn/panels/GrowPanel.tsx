...
// src/components/learn/panels/GrowPanel.tsx — XP, badges, streak + daily test UI.
...
export function GrowPanel({ courseId, xpTotal, learnerLevel, streak, onXPChange }: Props) {
 ...
 <StatCard icon={<Star className="h-4 w-4 text-growth-amber" />} label="XP" value={xpTotal} />
 <StatCard icon={<Trophy className="h-4 w-4 text-primary" />} label="Level" value={learnerLevel} />
 <StatCard icon={<Flame className="h-4 w-4 text-growth-amber" />} label="Streak" value={`${streak}d`} />
 </section>
 ...
 <span className="ml-2 text-[10px] uppercase tracking-wide text-growth-amber">Spaced repetition</span>
 )}
 </span>
 ...
 {a.evaluation === "correct" && <CheckCircle2 className="h-4 w-4 text-growth-sage" />}
 {a.evaluation === "partial" && <AlertCircle className="h-4 w-4 text-growth-amber" />}
 {a.evaluation === "incorrect" && <XCircle className="h-4 w-4 text-destructive" />}
