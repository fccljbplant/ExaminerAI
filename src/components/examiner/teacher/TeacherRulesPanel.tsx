...
/**
 * TeacherRulesPanel — configurable personal rules.
 *
...
export function TeacherRulesPanel() {
  ...
  const load = () => {
    setLoading(true);
    api.get<{ rules: Rule[] }>("/api/teacher/rules")
  ...
  const create = async () => {
    setCreating(true);
    try {
      await api.post("/api/teacher/rules", { condition, action });
      load();
    } catch { }
  ...
  const remove = async (id: string) => {
    try {
      await api.del(`/api/teacher/rules?id=${id}`);
